    ...
    void MassSpringSystemT<MassSpringProblem>::setup_spring_graph() {
        ...
        for (size_t j(0); j <= n_grid_y_; j++) {
            for (size_t i(0); i <= n_grid_x_; i++) {
                ...
                if (i > 0) {
                    // add horizontal
                    sg_.add_edge(j*(n_grid_x_+1) +i-1, j*(n_grid_x_+1) +i, 1, sqrt(2));

                    if (j > 0)
                        // add diagonal '/'
                        sg_.add_edge((j-1)*(n_grid_x_+1) +i-1, j*(n_grid_x_+1) +i, 1, sqrt(2));
                }
                if (j > 0) {
                    // add vertical
                    sg_.add_edge((j-1)*(n_grid_x_+1) +i, j*(n_grid_x_+1) +i, 1, sqrt(2));
