    ...
    public:
        ...
        virtual double eval_f(const Vec &_x) override {
            ...
            for (size_t i(0); i < springs_.size(); i++) {
                size_t pos_x(2*springs_[i].first), pos_y(2*springs_[i].second);
                xe_ << _x[pos_x], _x[pos_x+1], _x[pos_y], _x[pos_y+1];
                ...
                for (size_t i(0); i < 4; i++) {
                    for (size_t j(0); j < 4; j++) {
                        triplets.push_back(T(
                            std::vector<size_t>({pos_x, pos_x+1, pos_y, pos_y+1})[i],
                            std::vector<size_t>({pos_x, pos_x+1, pos_y, pos_y+1})[j],
                            he_(i, j)
                        ));
                    }
                }
