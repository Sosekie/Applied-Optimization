...
void MassSpringSystemT<MassSpringProblem>::add_constrained_spring_elements(const int _scenario) {
    ...
    if (_scenario == 1)
    {
        msp_.get()->add_spring_element(get_grid_index(0, 0), get_grid_index(0, 0), wn, 0); // Top-left corner to (0,0)
        msp_.get()->add_spring_element(get_grid_index(n_grid_x_, 0), get_grid_index(n_grid_x_, 0), wn, 0); // Top-right corner to (2m,0)
        msp_.get()->add_spring_element(get_grid_index(0, n_grid_y_), get_grid_index(0, n_grid_y_), wn, 0); // Bottom-left corner to (0,2n)
        msp_.get()->add_spring_element(get_grid_index(n_grid_x_, n_grid_y_), get_grid_index(n_grid_x_, n_grid_y_), wn, 0); // Bottom-right corner to (2m,2n)
    }
    else if (_scenario == 2)
    {
        for (int i = 0; i <= n_grid_x_; ++i) {
            msp_.get()->add_spring_element(get_grid_index(i, 0), get_grid_index(i, 0), wn, 0); // Side nodes to (i,0)
            msp_.get()->add_spring_element(get_grid_index(i, n_grid_y_), get_grid_index(i, n_grid_y_), wn, 0); // Side nodes to (i,2n)
        }
    }
