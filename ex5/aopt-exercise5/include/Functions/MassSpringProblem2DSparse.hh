    ...
    public:
        ...
        virtual double eval_f(const Vec &_x) override {
            ...
            for(size_t i = 0; i < attached_node_indices_.size(); ++i) {
                ...
                Vec coeffs(3);
                coeffs[0] = weights_[i];
                coeffs[1] = desired_points_[2 * i];
                coeffs[2] = desired_points_[2 * i + 1];

                energy += cse_.eval_f(cs_xe_, coeffs);
            }
        ...
        virtual void eval_gradient(const Vec &_x, Vec &_g) override {
            ...
            for(size_t i = 0; i < attached_node_indices_.size(); ++i) {
                ...
                // Get local gradient
                cse_.eval_gradient(cs_xe_, coeffs, cs_ge_);
        ...
        virtual void eval_hessian(const Vec &_x, SMat& _h) override {
            ...
            for(size_t i = 0; i < attached_node_indices_.size(); ++i) {
                int id0 = 2 * attached_node_indices_[i];
                int id1 = 2 * attached_node_indices_[i] + 1;
                ...
                // Get local Hessian
                cse_.eval_hessian(cs_xe_, coeffs, cs_he_);
